...
- Linear: y = x @ W + b, with He initialization. Backward computes dW, db and dx.
- ReLU: y = max(0, x). Backward passes the gradient only where x > 0.
- SoftmaxCrossEntropy: softmax + cross-entropy loss. Forward subtracts each row's max
  before exp so it never overflows; backward is (probs - one_hot) / batch_size.

Every backward pass is verified numerically in gradcheck.py.
"""
